...
public:
  ...
  void my_udp_receive()
  {
      ...
      [this] (boost::system::error_code ec, std::size_t recvd_bytes) {
          if ( !ec && recvd_bytes > 0 ) {
            std::cout << "[" << recvd_bytes << "] " << buffer_ << std::endl;
            jaj joj;
            memcpy(&joj, buffer_, sizeof(jaj));
            std::cout << joj.a << "| |" <<  joj.b << "| |" <<  joj.c << "|" << std::endl;
            my_udp_send_back();
          }
          else {
            my_udp_receive();
...
private:
  ...
  char data_[sizeof(jaj)];
  char buffer_[sizeof(jaj)];
