...
void Particle::drawSFML(sf::RenderWindow &window){
...
void ParticleSystem::drawRaylib(){
    for (std::vector<Particle>::iterator obj = system.begin(); obj < system.end(); obj++) {
        obj->drawRaylib();
    }
}

void ParticleSystem::drawSFML(sf::RenderWindow &window){
    for (std::vector<Particle>::iterator obj = system.begin(); obj < system.end(); obj++) {
        obj->drawSFML(window);
    }
}
